    ...
    def Newmark(self, dt, T, IC, tol_res=1e-6, tol_g=1e-6, gamma=1/2+1e-2, beta=1/4+1e-3):
        ...
        for n in range(n_steps-1):
            ...
            for i in range(max_iter):
                ...
                C_t_mat = C_t(q[:, n+1], dq[:, n+1], lambdas[:, n+1], self.p)
                K_t_mat = K_t(q[:, n+1], lambdas[:, n+1], self.p)
                ...
                S_11 = 1/(beta * dt**2) * M + gamma / \
                    (beta*dt) * C_t_mat + K_t_mat
                S_12 = k * G_guess.T
                S_21 = k*G_guess
                S_22 = np.zeros((n_const, n_const))

                delta_q, delta_lambdas = self.SolveWitPreconditioning(
